...
def gen_geometric_progression(start, end, n) -> torch.Tensor:
    ratio = (end / start) ** (1/n)
    ratio = torch.tensor(ratio)
    powers = torch.arange(n+1)
    ratios = ratio.repeat(n+1) ** powers
    rs =  start * ratios
    return rs


def perturb_data(X, rs):
    return X + torch.einsum('b,bcwh->bcwh', rs, torch.randn_like(X))
